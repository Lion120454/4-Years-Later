...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        std::cout << "Процесс 1: Левый треугольник матрицы:" << std::endl;
        print_matrix(recv_matrix, N, N, true);
        MPI_Send(recv_matrix, 1, triangle_type, 2, 0, MPI_COMM_WORLD);
    }
    else if (rank ==2){
        // Прием данных в процесс 2
        MPI_Recv(select_vector, T, MPI_INT, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ...
        for (int i = 0; i < T; ++i)
        {
                std::cout << select_vector[i] << " ";
        }
        std::cout << std::endl;
    }

    if (rank == 2) {
        // Отправка данных с второго в нулевой процесс
        MPI_Send(select_vector, T, MPI_INT, 0, 0, MPI_COMM_WORLD);
    } else if (rank == 0) {
        // Прием данных в процесе 0
